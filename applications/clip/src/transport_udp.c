...
LOG_MODULE_REGISTER(transport_udp, CONFIG_CLIP2_LOG_LEVEL);

/* Frame types (must match wifi_udp.c) */
...
/* UDP configuration */
#define UDP_MAX_DATA_SIZE   480   /* Max data per packet (within typical MTU) */
...
static int wait_for_ack(uint16_t expected_seq)
{
    ...
    while (retries < UDP_MAX_RETRIES) {
        ...
        if (ret == 0) {
            retries++;
            LOG_WRN("ACK timeout, retry %d/%d", retries, UDP_MAX_RETRIES);
            continue;
        }
...
static int udp_send_file_data(const uint8_t *data, uint16_t len)
{
    ...
    /* Total size: 5 bytes header + len bytes data */
    uint8_t frame[5 + 1024];  /* Max chunk size */
    if (len > 1024) {
        len = 1024;
    }
...
static int udp_send_file_start(const char *session_id, const char *filename, uint32_t size)
{
    ...
    /* Don't wait for ACK - unreliable but faster */
    return udp_sendto(frame, 8 + fn_len) >= 0 ? 0 : -1;
}
...
static int udp_send_file_end(const char *filename)
{
    ...
    /* Build frame: type(1) + seq(2) + fn_len(1) + filename */
    uint8_t frame[64];
    ...
    /* Don't wait for ACK */
    int ret = udp_sendto(frame, 4 + fn_len);
...
static int udp_send_transfer_done(const char *session_id, uint32_t file_count)
{
    ...
    /* Don't wait for ACK */
    return udp_sendto(frame, 8 + sid_len) >= 0 ? 0 : -1;
}
