...
## 9. Competitor Benchmark — 安克AI录音豆 D3200

### 9.1 竞品规格对比

| 维度 | ReSpeaker Clip | 安克AI录音豆 D3200 |
|------|---------------|-------------------|
| **价格** | TBD（低成本定位） | 899 RMB |
| **形态** | 夹子式（~40×20mm） | 纽扣式（Φ23.2mm, 10g） |
| **主控** | nRF5340 (双核 ARM) | BES2710IMP (恒玄蓝牙 SoC) |
| **WiFi** | nRF7002 (WiFi AP) | RTL8711DAF (瑞昱) |
| **存储** | SD 卡 (FAT) | eMMC 5.1 8GB (康盈KAS0311) |
| **麦克风** | 2× PDM MEMS | 2× MEMS (镭雕SA12) |
| **降噪** | SpeexDSP (软件) | 自研智能降噪 (BES SoC 内置) |
| **编码** | Opus 24kbps | 未知（推测 AAC/Opus） |
| **电池** | 200mAh (NPM1300 PMIC) | 90mAh + 400mAh 充电盒 |
| **续航** | ~16h 录音 | 8h + 充电盒 32h |
| **拾音距离** | 目标 3m | 标称 5m |
| **传输** | WiFi AP (UDP) + BLE | 蓝牙 + WiFi 快传 |
| **AI 平台** | 自研 | 飞书深度整合 |
| **防水** | 无 | IPX4 |
| **振动反馈** | 有 (PMIC GPIO) | 有 (振动马达) |

> 数据来源：[52audio 拆解报告](https://www.52audio.com/archives/269266.html)

### 9.2 统一测试方法

**核心原则：同环境、同语料、同流程**

- 同一房间、同一天、**2 小时内**完成所有测试，避免环境条件变化
- 同一扬声器播放 Section 4 的参考语料（中文 20 句 + 英文 20 句 + 数字 10 组）
- 扬声器音量：**65 dB(A) @ 1m**（模拟正常说话音量）
- Clip 和安克录音豆**并排放置**在相同距离，麦克风朝向声源
- 各自用**最佳录音模式**：Clip Enhanced mode，安克默认模式
- 测试距离：**0.3m / 1m / 2m / 3m / 5m**（5m 测试安克标称极限）

### 9.3 竞品录音导出方法

| 设备 | 导出方式 | 转换方法 |
|------|---------|---------|
| Clip | UDP sync 或 SD 卡读卡器 | `decode_opus.py` 或 ffmpeg 转 16kHz mono WAV |
| 安克录音豆 | WiFi 快传导出原始录音文件 | ffmpeg 转 16kHz mono WAV |
...
# 如果安克录音是多通道，取左声道
ffmpeg -i input.xxx -ar 16000 -ac 1 -af "pan=mono|c0=FL" output.wav
```

### 9.4 对比数据表

每个场景测试 5 个指标：SNR / STOI / PESQ / WER(CN) / WER(EN)

#### 安静场景

| 场景 | 指标 | Clip Enhanced | 安克录音豆 | 差值 | 备注 |
|------|------|--------------|-----------|------|------|
| S1 安静 0.3m | SNR | -- | -- | -- | |
| S1 安静 0.3m | STOI | -- | -- | -- | |
| S1 安静 0.3m | PESQ | -- | -- | -- | |
...
| S3a 安静 3m | WER(CN) | -- | -- | -- | |
| S3a 安静 3m | WER(EN) | -- | -- | -- | |
| S3b 安静 5m | SNR | -- | -- | -- | 安克标称 5m |
| S3b 安静 5m | STOI | -- | -- | -- | 安克标称 5m |
| S3b 安静 5m | PESQ | -- | -- | -- | 安克标称 5m |
| S3b 安静 5m | WER(CN) | -- | -- | -- | 安克标称 5m |
| S3b 安静 5m | WER(EN) | -- | -- | -- | 安克标称 5m |
...
> 差值 = Clip 值 - 安克值（正值表示 Clip 优，SNR/PESQ/STOI 越大越好，WER 越小越好需取反）

#### 额外对比项

| 对比项 | Clip | 安克录音豆 | 备注 |
|--------|------|-----------|------|
| 录音启动延迟 | -- | -- | 从按键到开始录音的实际延迟 |
| 文件大小 | -- | -- | 同一段录音的文件体积（编码效率） |
| 频响范围 | -- | -- | Audacity 频谱图截图并排 |

### 9.5 雷达图评分维度

| 维度 | 计算方法 | 权重 |
|------|---------|------|
| 近场清晰度 | S1 的 (PESQ + STOI) / 2，归一化到 0-5 | 20% |
| 远场拾音 | S2 + S3 的 STOI 平均值，归一化到 0-5 | 20% |
| 极限距离 | S3a(3m) + S3b(5m) 的 WER(CN)，越低越好，归一化到 0-5 | 15% |
| 抗噪能力 | S4 + S5 + S7 的 WER(CN) 平均，越低越好，归一化到 0-5 | 25% |
| 编码效率 | 同内容文件大小，越小越好但需保持质量，归一化到 0-5 | 10% |
| 续航能力 | 标称录音时长，归一化到 0-5 | 10% |
...
脚本位置：`applications/clip/tests/tools/audio_benchmark.py`

### 10.1 使用方法

```bash
# 单文件测试
python applications/clip/tests/tools/audio_benchmark.py \
  --ref reference.wav \
  --test recording.wav \
  --output result.json
...
```bash
pip install pystoi pesq openai-whisper jiwer numpy scipy
```
...
- [ ] Clip 固件版本已记录
- [ ] 安克录音豆固件版本已记录（APP 查看）
- [ ] 参考音频已准备（Section 4 语料，16kHz WAV）
- [ ] 两台设备并排放置，麦克风朝向声源

### 11.2 测试执行

- [ ] 同一轮测试 2 小时内完成
- [ ] 每个场景先录 Clip，再录安克（或同时录，避免时间差）
- [ ] 每段录音前后各留 2 秒静音，便于对齐
- [ ] 记录每个场景的实际环境噪声水平

### 11.3 数据处理

- [ ] 所有录音导出后统一转 16kHz mono WAV
- [ ] 使用 `audio_benchmark.py` 批量计算客观指标
- [ ] ASR 转写使用同一模型（whisper-large-v3）
- [ ] 填写 Section 9.4 对比数据表

### 11.4 结果输出

- [ ] 生成雷达图（matplotlib）
- [ ] 填写定量结论模板（Section 9.6）
- [ ] 频谱截图（Audacity）并排对比
