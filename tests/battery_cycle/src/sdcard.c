...
/* ============================================================================
 * Continuous SD READ = a discharge load. Writes a fixed 1 MB file ONCE
 * (lazy, on first enable) so the rest is pure read — no NAND write wear. A
 * thread then reads it in a loop (seeking back at EOF) while the load is
 * active. Adds ~15-25 mA (card read + SPI4 + CPU) on top of the WiFi TX load.
 * ============================================================================ */
#define DISCHARGE_FILE      "/SD:/discharge.bin"
#define DISCHARGE_FILE_SIZE (1U * 1024 * 1024)   /* 1 MB */
#define DISCHARGE_CHUNK     4096

static volatile bool sd_discharge_active;
static bool discharge_file_ready;

/* Lazily create the read file (one-time write) + auto-mount if needed. After
 * the first call the file exists and this is a no-op. */
static int ensure_discharge_file(void)
{
	int rc;
	struct fs_file_t file;
	static uint8_t wbuf[DISCHARGE_CHUNK];

	if (discharge_file_ready) {
		return 0;
	}
	if (!sd_mounted) {
		(void)sdcard_mount();  /* best-effort auto-mount */
	}
	if (!sd_mounted) {
		return -ENODEV;
	}
	...
	fs_file_t_init(&file);
	rc = fs_open(&file, DISCHARGE_FILE, FS_O_CREATE | FS_O_WRITE);
	if (rc != 0) {
		LOG_WRN("discharge file create failed: %d", rc);
		return rc;
	}
	for (uint32_t off = 0; off < DISCHARGE_FILE_SIZE; off += DISCHARGE_CHUNK) {
		rc = fs_write(&file, wbuf, DISCHARGE_CHUNK);
		if (rc < 0) {
			fs_close(&file);
			LOG_WRN("discharge file write failed: %d", rc);
			return rc;
		}
	}
	fs_close(&file);
	discharge_file_ready = true;
	LOG_INF("discharge file ready (%u KB, write-once)", DISCHARGE_FILE_SIZE / 1024);
	return 0;
}

static void sd_discharge_thread(void *a, void *b, void *c)
{
	ARG_UNUSED(a);
	ARG_UNUSED(b);
	ARG_UNUSED(c);

	static uint8_t rbuf[DISCHARGE_CHUNK];

	for (;;) {
		if (sd_discharge_active && sd_mounted) {
			struct fs_file_t file;
			fs_file_t_init(&file);
			if (fs_open(&file, DISCHARGE_FILE, FS_O_READ) == 0) {
				/* Read to EOF, seek back, repeat — pure read, no wear. */
				while (sd_discharge_active) {
					ssize_t n = fs_read(&file, rbuf, DISCHARGE_CHUNK);
					if (n <= 0) {
...
K_THREAD_DEFINE(sd_discharge_tid, 2048, sd_discharge_thread,
		NULL, NULL, NULL, K_LOWEST_APPLICATION_THREAD_PRIO, 0, 0);

int sdcard_discharge_load_enable(bool enable)
{
	if (enable) {
		/* One-time write of the read file (instant after the first call). */
		(void)ensure_discharge_file();
		sd_discharge_active = true;
		LOG_INF("SD discharge load: ON");
	} else {
		sd_discharge_active = false;
	}
	return 0;
}
