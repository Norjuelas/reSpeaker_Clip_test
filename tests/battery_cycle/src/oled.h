 ...
 *
 * Renders four lines: "Bat: NN%", the charge/discharge state with the cycle
 * count (e.g. "DISCHARGE #5"), the battery voltage in mV, and the battery
 * temperature.
 *
 * @param pct Battery percentage (0-100)
 * @param charging True if charging
 * @param mv Battery voltage in millivolts
 * @param temp_c Battery temperature in degrees Celsius
 * @param state State string, e.g. "CHARGE" / "DISCHARGE"
 * @param cycles Completed charge/discharge cycle count
 */
void oled_show_battery(uint8_t pct, bool charging, uint32_t mv, int32_t temp_c,
