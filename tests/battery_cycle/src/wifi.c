...
static int do_wifi_ap_stop(void)
{
	...
	/* Bring the whole interface down so the nRF70 radio powers off (and, with
	 * CONFIG_NRF70_QSPI_LOW_POWER, the QSPI link drops to low power). Just
	 * disabling the AP leaves the interface admin-up and the radio awake,
	 * which keeps drawing power during the charge phase and slows charging.
	 * do_wifi_ap_start() brings the interface back up on the next discharge. */
	(void)net_if_down(iface);

	printk("[WiFi] AP stopped (radio off)\n");
	return 0;
}
